% skeleton "lalr1.cc" // -*- C++ -*-
    % require "3.8.2" % header

    % define api.token.raw % define api.token.constructor %
    define api.value.type variant % define parse.assert % define parse.trace %
    define parse.error detailed % define parse.lac full
...
#include <exception>
    ...
    token ASSIGN "=" MINUS "-" PLUS "+" STAR "*" SLASH "/" LPAREN "(" RPAREN
                 ")" READ "?" PRINT "print" LCPAREN "{" RCPAREN "}" SEMIC ";" IF
                 "if" ELSE "else" ELSEIF "else if" WHILE "while" GREATER
                 ">" LESS "<" GREATER_E ">=" LESS_E "<=" EQUAL "==" NOT_EQUAL
                 "!=" NOT "!" MOD "%" AND "&&" OR "||";
...
% token<std::string> ID "identifier" %
    ...
    // ----- Statement derived -----
    % nterm<AST::ExpressionNode *> Expr % nterm<AST::UnaryOpNode *> UnaryOp %
    nterm<AST::BinaryOpNode *> BinaryOp % nterm<AST::AssignNode *> Assign %
    nterm<AST::ScopeNode *> Scope % nterm<AST::PrintNode *> Print %
    nterm<AST::IfNode *> If_Stm % nterm<AST::ElseLikeNode *> Else_Like %
    nterm<AST::ElseNode *> Else % nterm<AST::ElseIfNode *> Else_If %
    nterm<AST::WhileNode *> While_Stm % nterm<AST::VariableNode *> Variable
...
% nonassoc "if" % nonassoc "print" % left "=" %
    left "+"
         "-" %
    left "*"
         "/" %
    ...
    Program : Statements YYEOF
{
    ...
    drv.ast.globalScope = drv.ast.construct<AST::ScopeNode>(
        std::move(drv.stm_table[drv.cur_scope_id]));
};
...
Scope : StartScope Statements EndScope
{
    ...
    $$ = drv.ast.construct<AST::ScopeNode>(
        std::move(drv.stm_table[drv.cur_scope_id]));
...
Else : ELSE Statement { $$ = drv.ast.construct<AST::ElseNode>($2); }
