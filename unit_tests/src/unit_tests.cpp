...
TEST(AST, assign)
{
    ...
    auto assign = std::make_unique<AST::AssignNode>(std::move(x), std::move(constant));
...
TEST(AST, checkAddition)
{
    ...
    std::unique_ptr<AST::INode> sum = std::make_unique<AST::BinaryOpNode>(std::make_unique<AST::ConstantNode>(1000),
                                                                          AST::BinaryOp::ADD,
